...
app.get('/personal', verifyUser, (req, res) => {
  res.json({ Status: "Success", name: req.name });
});

app.get('/workspace', verifyUser, (req, res) => {
  res.json({ Status: "Success", name: req.name });
});

app.get('/calender', verifyUser, (req, res) => {
  res.json({ Status: "Success", name: req.name });
});

app.get('/to-do-list', verifyUser, (req, res) => {
  res.json({ Status: "Success", name: req.name });
});

app.get('/gantchart', verifyUser, (req, res) => {
  res.json({ Status: "Success", name: req.name });
});

app.get('/notepad', verifyUser, (req, res) => {
  res.json({ Status: "Success", name: req.name });
});
...
app.post('/cards', (req, res) => {
  const { list_id, title, content, labels } = req.body;
  db.query('INSERT INTO cards (list_id, title, content, labels) VALUES (?, ?, ?, ?)', [list_id, title, content, labels], (err, result) => {
      if (err) {
          return res.status(500).json({ error: err.message });
      }
      res.json({ id: result.insertId, list_id, title, content, labels });
  });
});
...
app.put('/cards/:id/update', (req, res) => {
  const { id } = req.params;
  const { title, content, list_id, labels, completion_date, is_completed } = req.body;
  ...
  const query = `
      ...
      WHERE id = ?
  `;
  const params = [title, content, list_id, labels, completion_date, is_completed, id];
...
//-------------------deletion of cards,lists and boards-------------------------
...
app.get('/cards/:cardId/checklists', (req, res) => {
  const { cardId } = req.params;
  const query = 'SELECT * FROM checklists WHERE card_id = ?';
...
app.post('/checklists', (req, res) => {
  ...
  const query = 'INSERT INTO checklists (card_id, content) VALUES (?, ?)';
  db.query(query, [card_id, content], (err, results) => {
      if (err) {
          console.error(err.message);
          return res.status(500).send('Server error');
      }
      const newChecklistId = results.insertId;
      db.query('SELECT * FROM checklists WHERE id = ?', [newChecklistId], (err, results) => {
          if (err) {
              console.error(err.message);
              return res.status(500).send('Server error');
          }
...
app.put('/checklists/:id', (req, res) => {
  ...
  const query = 'UPDATE checklists SET content = ?, is_completed = ? WHERE id = ?';
  db.query(query, [content, is_completed, id], (err, results) => {
      if (err) {
          console.error(err.message);
          return res.status(500).send('Server error');
      }
      db.query('SELECT * FROM checklists WHERE id = ?', [id], (err, results) => {
          if (err) {
              console.error(err.message);y
              return res.status(500).send('Server error');
          }
          res.json(results[0]);
      });
...
app.put('/cards/:id/completion', (req, res) => {
  const { id } = req.params;
  const { completion_date, is_completed } = req.body;
  const query = 'UPDATE cards SET completion_date = ?, is_completed = ? WHERE id = ?';
  
  // If completion_date is empty, set it to NULL
  const dateValue = completion_date ? completion_date : null;

  db.query(query, [dateValue, is_completed, id], (err, results) => {
    if (err) {
      console.error(err.message);
      return res.status(500).send('Server error');
    }
    res.json({ message: 'Completion date and status updated successfully' });
  });
});


app.get('/cards/:id/completion', (req, res) => {
  const { id } = req.params;
  const query = 'SELECT completion_date, is_completed FROM cards WHERE id = ?';
  ...
  db.query(query, [id], (err, results) => {
    ...
    res.json({ completion_date: card.completion_date, is_completed: card.is_completed });
  });
});
...
//------------------------gant chart tasks--------------------------------------------------------// Get all tasks
...
app.post('/api/documents', (req, res) => {
  ...
  const query = 'INSERT INTO documents (title, content, email) VALUES (?, ?, ?)';
  db.query(query, [title, content, email], (err, result) => {
      if (err) {
          return res.status(500).send(err);
      }
      res.send({ id: result.insertId, message: 'Document created successfully' });
  });
});
...
app.put('/api/documents/:id', (req, res) => {
  const { id } = req.params;
  const { title, content, email } = req.body;
  const query = 'UPDATE documents SET title = ?, content = ? WHERE id = ? AND email = ?';
  db.query(query, [title, content, id, email], (err, result) => {
      if (err) {
          return res.status(500).send(err);
      }
...
app.get('/api/documents/:id', (req, res) => {
  ...
  const query = 'SELECT * FROM documents WHERE id = ? AND email = ?';
  db.query(query, [id, email], (err, results) => {
      if (err) {
          return res.status(500).send(err);
      }
...
app.get('/api/documents', (req, res) => {
  const { email } = req.query;
  const query = 'SELECT id, title FROM documents WHERE email = ?';
  db.query(query, [email], (err, results) => {
      if (err) {
          return res.status(500).send(err);
      }
...
app.delete('/api/documents/:id', (req, res) => {
  const { id } = req.params;
  const { email } = req.query;
  const query = 'DELETE FROM documents WHERE id = ? AND email = ?';
